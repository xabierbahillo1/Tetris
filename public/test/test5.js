...
// ==================== Tetris ==========================
    ...
    Tetris.prototype.init = function(){
        ...
        this.board.draw_shape(this.current_shape);  //Lla
...
// clear the canvas
    ...
    QUnit.test('Testing movements', function(assert) {
        ...
        assert.pixelEqual(canvas, 15*3*3, 15, 0, 128, 0, 255, "Green al moverse a la izquierda");
        ...
        assert.pixelEqual(canvas, 15*3*3, 15, 255, 255, 255, 255, "White al moverla hacia abajo");


        // y el bloque que está 24 posiciones más abajo, verde

        assert.pixelEqual(canvas, 15*3*3, 15+24, 0, 128, 0, 255, "Green un poco más abajo");

        // si movemos la pieza 24 veces a la derecha, debe ser blanca otra vez
        for(i=1;i<=24;i++)
            game.do_move("Right");

        assert.pixelEqual(canvas, 15*3*3, 15, 255, 255, 255, 255, "White tras volver a mover la pieza hacia la derecha");
